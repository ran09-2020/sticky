...
## CSS / Tailwind v4
- Use Tailwind utility classes in components for ordinary styling.
...
- `src/custom.css` is an escape hatch. Leave it without active rules unless the user explicitly asks for custom/plain CSS or the requirement cannot be implemented with Tailwind utilities, variants, arbitrary values, or theme tokens.
- Before adding custom CSS, verify that Tailwind has no reasonable solution. Custom keyframes are an appropriate example because Tailwind utilities can apply an animation but cannot define its keyframe sequence.
- Put custom selectors and keyframes in `src/custom.css`; do not add them to `src/index.css` or `src/theme.css`.
